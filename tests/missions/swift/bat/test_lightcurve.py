...
ones_file = data_path / 'swift-bat'/ 'sw00974827000bev1s.lc.gz'
ms_file = data_path / 'swift-bat'/ 'sw00974827000bevms.lc.gz'
...
class TestBatLightcurve(unittest.TestCase):
    ...
    def test_write(self):
        with TemporaryDirectory() as this_path:
            self.lc.write(this_path, 'testfile.fits')
            phaii = BatLightcurve.open(os.path.join(this_path, 'testfile.fits'))
            ...
            # these will fail due to checksum
...
class TestBatLightcurve2(unittest.TestCase):
    ...
    def test_write(self):
        with TemporaryDirectory() as this_path:
            self.lc.write(this_path)
            phaii = BatLightcurve.open(os.path.join(this_path, self.lc.filename))
            counts_list = self.lc.data.counts.tolist()
            for i, counts in enumerate(phaii.data.counts.tolist()):
                self.assertListEqual(counts, counts_list[i])
            self.assertListEqual(phaii.data.tstart.tolist(), self.lc.data.tstart.tolist())
            self.assertListEqual(phaii.data.tstop.tolist(), self.lc.data.tstop.tolist())
            self.assertListEqual(phaii.data.exposure.tolist(), self.lc.data.exposure.tolist())
            self.assertListEqual(phaii.ebounds.low_edges(), self.lc.ebounds.low_edges())
