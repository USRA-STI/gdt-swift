...
class.
    ...
    >>> # read a 1s file
    >>> filepath = data_path.joinpath('swift-bat/sw00974827000bev1s.lc.gz')
...
We can also view the 64 ms binning lightcurve file with similar steps.
    ...
    >>> filepath = data_path.joinpath('swift-bat/sw00974827000bevms.lc.gz')
    >>> file = BatLightcurve.open(filepath)
    >>> lightcurve = file.to_lightcurve(energy_range=(15.0, 350.0))
    >>> lcplot = Lightcurve(data=lightcurve)
    >>> lcplot.errorbars.hide()
...
Similarly, we can plot the count spectrum:
    ...
    >>> specplot = Spectrum(data=file.to_spectrum())
    >>> plt.show()
...
.. automodapi:: gdt.missions.swift.bat.lightcurve
   :inherited-members:
